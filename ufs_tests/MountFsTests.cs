...
public class MountFsTests
{
    private readonly string testRoot = "/";
    private readonly MountFileSystem fs;

    public MountFsTests()
    {
        var memFs1 = new MemoryFileSystem(new MemoryFileTree.Root());
        var memFs2 = new MemoryFileSystem(new MemoryFileTree.Root());
        fs = new MountFileSystem();
        fs.Mount("/tmp".FsPath(), memFs1);
        fs.Mount("/".FsPath(), memFs2);
    }

    [Fact]
    public async Task Mount_ShouldWork()
    {
        await fs.WriteAllText("/tmp/test.txt".FsPath(), "Hello, World!");
        var content = await fs.ReadAllText("/tmp/test.txt".FsPath());
        Assert.Equal("Hello, World!", content);
        Assert.True(await fs.FileExists("/tmp/test.txt".FsPath()));
    }

    [Fact]
    public async Task File_On_Unmounted_ShouldNotWork()
    {
        await fs.WriteAllText("/tmp/unmounted.txt".FsPath(), "This should not be accessible after unmount.");
        fs.Unmount("/tmp".FsPath());
        await Assert.ThrowsAsync<FileSystemException.NotFound>(
            () => fs.ReadAllText("/tmp/unmounted.txt".FsPath()));
        Assert.False(await fs.FileExists("/tmp/unmounted.txt".FsPath()));
    }
